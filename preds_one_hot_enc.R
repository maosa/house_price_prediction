...
modify_data <- function(to_convert, original, final) {
  ...
  dmy <- dummyVars(" ~ .", data = chr_test[, updated_cols])
  
  trsf <- data.frame(predict(dmy, newdata = chr_test[, updated_cols]))
...
dmy <- dummyVars(" ~ .", data = chr_train[, updated_cols])

trsf <- data.frame(predict(dmy, newdata = chr_train[, updated_cols]))
...
# Remove any dots from the columns names

colnames(num_train) <- 
  str_replace_all(string = colnames(num_train), pattern = '\\.', replacement = '')

colnames(new_test) <- 
  str_replace_all(string = colnames(new_test), pattern = '\\.|\\.\\.', replacement = '')

colnames(num_train)[c(which(!colnames(num_train) %in% colnames(new_test)))]

num_train <- select(.data = num_train, -"ElectricalMix")

if(!all(colnames(num_train)[c(which(!colnames(num_train) %in% colnames(new_test)))] == "SalePrice")) {
  stop("ERROR!")
}
...
models <- c("xgbLinear", "rf", "xgbTree", "bstTree")
...
if (class(final_submission$Id) == "numeric" & class(final_submission$SalePrice) == "numeric") {
  ...
  write.table(x = final_submission,
              file = "SalePrice_ensemble_one_hot_enc_preds.csv",
